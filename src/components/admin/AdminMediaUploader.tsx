...
export function AdminMediaUploader({ profile, assets: initialAssets = [] }: { profile: any; assets?: any[] }) {
  ...
  return <section className="space-y-5"><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><h2 className="font-display text-2xl font-bold">Media Library</h2><p className="mt-1 text-sm text-muted-foreground">Upload images, certificates and CV files once, then reuse their URLs across CMS forms.</p></div><button onClick={()=>input.current?.click()} disabled={uploading} className="inline-flex items-center justify-center gap-2 rounded-xl bg-primary px-4 py-2.5 text-sm font-semibold text-primary-foreground"><Upload className="h-4 w-4"/>{uploading?'Uploading…':'Upload media'}</button><input ref={input} type="file" accept="image/*,.pdf,.doc,.docx" className="hidden" onChange={event=>event.target.files?.[0]&&upload(event.target.files[0])}/></div>
    <div className="rounded-2xl border border-border bg-card p-4"><h3 className="mb-3 text-sm font-semibold">Current branding</h3><div className="grid gap-3 sm:grid-cols-3">{[['Profile image',profile?.profileImageUrl],['Hero image',profile?.heroImageUrl],['Favicon',profile?.faviconUrl]].map(([label,url])=><div key={label} className="rounded-xl border border-border p-3"><p className="text-xs text-muted-foreground">{label}</p><p className="mt-1 truncate text-sm">{url||'Not set'}</p></div>)}</div></div>
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{assets.map((asset)=><article key={asset.id} className="overflow-hidden rounded-2xl border border-border bg-card">{asset.kind==='IMAGE'||asset.kind==='ICON'?<img src={asset.url} alt={asset.altText||asset.name} className="h-36 w-full bg-muted object-cover"/>:<div className="grid h-36 place-items-center bg-muted"><File className="h-10 w-10 text-primary"/></div>}<div className="p-4"><div className="flex items-start gap-2"><div className="min-w-0 flex-1"><p className="truncate text-sm font-medium">{asset.name}</p><p className="mt-1 text-xs text-muted-foreground">{asset.kind} · {asset.size?`${Math.round(asset.size/1024)} KB`:''}</p></div><button onClick={()=>copy(asset.url)} title="Copy URL" className="rounded-lg p-2 text-muted-foreground hover:bg-muted"><Copy className="h-4 w-4"/></button><button onClick={()=>remove(asset)} title="Delete" className="rounded-lg p-2 text-muted-foreground hover:bg-destructive/10 hover:text-destructive"><Trash2 className="h-4 w-4"/></button></div></div></article>)}{assets.length===0&&<div className="col-span-full rounded-2xl border border-dashed border-border p-10 text-center text-sm text-muted-foreground"><ImageIcon className="mx-auto mb-3 h-8 w-8 opacity-40"/>No uploaded media yet.</div>}</div>
  </section>
}
