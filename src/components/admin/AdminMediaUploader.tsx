  ...
  return (
    <section className="space-y-5">
      ...
      <div className="rounded-2xl border border-border bg-card p-4">
        ...
        <div className="grid gap-3 sm:grid-cols-3">{[['Profile image', profile?.profileImageUrl], ['Hero image', profile?.heroImageUrl], ['Favicon', profile?.faviconUrl]].map(([label, url]) => <div key={label} className="rounded-xl border border-border p-3"><p className="text-xs text-muted-foreground">{label}</p><p className="mt-1 truncate text-sm">{url || 'Not set'}</p></div>)}</div>
      </div>
